...
char Lexer::jump(const int &offset){
    return source[position += offset];
}
void Lexer::newline() {
  this->line++;
...
void LexerError::report() {
  std::cerr << "LexerError at " << __FILE__ << "\n";
...
Token Tokenizer::number(){
    ...
    if (ch == '.'){
        if (std::isdigit(lexer.look(1))){
            ...
            LexerError(ErrorType::FaultyFloat, "there is a no number after '.'", lexer.line, lexer.column).report();
        }
    }
    if (isFloat){
...
Token Tokenizer::alpha(){
    ...
    if (std::isdigit(ch) || ch == '_'){ //is parsedString end with '_' or number?
...
Token Tokenizer::string(){
    ...
    if (ch == '\0'){
        LexerError(
            ErrorType::FaultyString,
            "there is a string that is not closed",
            lexer.line,
            lexer.column
        ).report();
    }
...
Token Tokenizer::op(){
    ...
    switch (ch){
        ...
        case '*': return Token(TokenType::MULT, "*", lexer.line, lexer.column);
        case '/': return Token(TokenType::DIVIDE, "/", lexer.line, lexer.column);
        case '%': return Token(TokenType::MODULO, "%", lexer.line, lexer.column);
        ...
        default:
            LexerError(ErrorType::InvalidSymbol, "there is a invalid symbol", lexer.line, lexer.column).report();
...
Token Tokenizer::punctuation(){
    char ch = lexer.peek();
    switch (ch) {
        case '(': lexer.advance(); return Token(TokenType::LPAREN, "(", lexer.line, lexer.column);
        case ')': lexer.advance(); return Token(TokenType::RPAREN, ")", lexer.line, lexer.column);
        case '[': lexer.advance(); return Token(TokenType::LBRACKET, "[", lexer.line, lexer.column);
        case ']': lexer.advance(); return Token(TokenType::RBRACKET, "]", lexer.line, lexer.column);
        case '{': lexer.advance(); return Token(TokenType::LBRACE, "{", lexer.line, lexer.column);
        case '}': lexer.advance(); return Token(TokenType::RBRACE, "}", lexer.line, lexer.column);
        case '.': lexer.advance(); return Token(TokenType::DOT, ".", lexer.line, lexer.column);
        case ',': lexer.advance(); return Token(TokenType::COMMA, ",", lexer.line, lexer.column);
        case ':': lexer.advance(); return Token(TokenType::COLON, ":", lexer.line, lexer.column);
        case ';': lexer.advance(); return Token(TokenType::SEMICOLON, ";", lexer.line, lexer.column);
        default: LexerError(ErrorType::UnexpectedToken, "there is a unexpected token", lexer.line, lexer.column).report();
    }
    return Token(TokenType::SKIP, "", lexer.line, lexer.column);
...
// TEST
int main() {
  std::string code = "id x = 3.14 \n function -> x, y {\nreturn x + y\n}";
  Tokenizer tokenizer(code);
  tokenizer.tokenize().show();
